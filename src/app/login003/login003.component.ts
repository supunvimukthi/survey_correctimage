...
export class Login003Component implements OnInit {
  ...
  ngOnInit() {
    ...
    this.text_mid="You have finished "+this.batch+" batch of 50 images! There one "+(4-Number(this.batch))+" more to go. We thank you again for assisting us with this task. You may take a break and return to this screen, or keep going immediately. Press the \"continue\" button to proceed."
    if (this.batch==undefined||null){
      console.log("test")
      this.ref=this.userData.push()
      localStorage.setItem("ref",JSON.stringify(this.ref))
    }
    if(this.batch!=undefined && this.batch!="04"){
      this.button_title="Continue"
      this.text=this.text_mid
    }else if(this.batch=="04"){
