...
export class Login004Component implements OnInit {
  ...
  ngOnInit() {
    ...
    this.text_mid="You have finished "+this.batch+" batches of 50 images! There are "+(3-Number(this.batch))+" more to go. We thank you again for assisting us with this task. You may take a break and return to this screen, or keep going immediately. Press the \"continue\" button to proceed."
  ...
  login(): void {
    if (true) {
      ...
      this.router.navigate(["image004"],{state:{"batch":Number(localStorage.getItem("batch"))?Number(localStorage.getItem("batch"))+1<10?"0"+String(Number(localStorage.getItem("batch"))+1):String(Number(localStorage.getItem("batch"))+1):"01"}});
    } else {
