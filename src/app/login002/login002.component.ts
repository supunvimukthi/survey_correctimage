...
@Component({
  selector: 'app-login002',
  templateUrl: './login002.component.html',
  styleUrls: ['./login002.component.css']
})
export class Login002Component implements OnInit {
  ...
  ngOnInit() {
    ...
    this.userData = this.db.database.ref('UserData002');
    this.batch=localStorage.getItem("batch002")
  ...
  login(): void {
    if (true) {
      ...
      this.router.navigate(["image002"],{state:{"batch":Number(localStorage.getItem("batch002"))?Number(localStorage.getItem("batch002"))+1<10?"0"+String(Number(localStorage.getItem("batch002"))+1):String(Number(localStorage.getItem("batch002"))+1):"01"}});
    } else {
