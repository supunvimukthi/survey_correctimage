...
export class Login002Component implements OnInit {
  ...
  login(): void {
    if (true) {
      ...
      this.router.navigate(["image002"],{state:{"batch":Number(localStorage.getItem("batch"))?Number(localStorage.getItem("batch"))+1<10?"0"+String(Number(localStorage.getItem("batch"))+1):String(Number(localStorage.getItem("batch"))+1):"01"}});
    } else {
